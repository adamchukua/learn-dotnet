...
<ul>
  ...
  <li>
    <a href="#lab9">Лабораторна робота 9</a>
  </li>
  
  <li>
    <a href="#lab10">Лабораторна робота 10</a>
  </li>
</ul>
<div id="lab1">
...
<div id="lab5">
  ...
  <ol>
    <li>Що таке HTTP? <br> HTTP (Hypertext Transfer Protocol) - це протокол передачі гіпертекстових документів в мережі Інтернет. Він використовується для передачі даних між вебсервером та клієнтом, який може бути браузером або будь-якою іншою програмою. </li>
    ...
    <li>На які групи поділяють програмне забезпечення для роботи з протоколом HTTP? <br> Програмне забезпечення для роботи з протоколом HTTP поділяють на дві групи: <ul>
        <li>Серверне програмне забезпечення - вебсервери, які надають вебсторінки клієнтам через HTTP.</li>
        <li>Клієнтське програмне забезпечення - веббраузери, які взаємодіють з вебсерверами за допомогою HTTP.</li>
      </ul>
    </li>
...
<div id="lab6">
  ...
  <ol>
  <li>Для чого використовується Swagger?<br>Swagger використовується для автоматичної генерації документації для RESTful вебсервісів. Він дозволяє описати структуру API, його ресурси, методи та параметри, тим самим полегшуючи розробку та спрощуючи розуміння вебсервісу.</li>
  <li>Як згенерувати колекцію для Postman, використовуючи Swagger?<br>Щоб згенерувати колекцію для Postman за допомогою Swagger, потрібно відкрити файл Swagger у веббраузері та натиснути на кнопку "Generate Collection". Після цього можна завантажити файл згенерованої колекції та імпортувати його в Postman.</li>
  <li>Що таке OpenAPI і яке його призначення?<br>OpenAPI (раніше відомий як Swagger) - це незалежний від вендора формат опису вебсервісів, який дозволяє автоматично генерувати документацію та різні інструменти для тестування вебсервісів. Його призначення полягає в упрощенні процесу розробки вебсервісів та полегшенні спілкування між розробниками та користувачами вебсервісу.</li>
...
REST (Representational State Transfer) API - це архітектурний стиль для створення вебсервісів, який використовує протокол HTTP для обміну даними між клієнтом та сервером.</li>
<li>У чому різниця між REST і RESTful?<br>
RESTful - це термін, який використовується для опису вебсервісів, які повністю відповідають принципам REST. Різниця між REST і RESTful полягає у тому, що REST є архітектурним стилем, тоді як RESTful є реалізацією цього стилю.</li>
...
REST API - це архітектурний стиль для створення вебсервісів, який використовує протокол HTTP для обміну даними між клієнтом та сервером, тоді як JSON API - це специфікація для форматування даних, яка забезпечує консистентність даних між клієнтом та сервером, а також оптимізує кількість запитів до сервера.</li>
...
<div id="lab8">
  ...
  OAuth - це відкритий протокол авторизації, який використовується для забезпечення стороннього доступу до ресурсів вебсайту без викладення логіну та пароля користувача. Цей протокол забезпечує авторизацію через сторонні сервіси, такі як Google або Facebook, і дозволяє користувачам давати стороннім сервісам обмежений доступ до їхніх особистих даних без надання доступу до їх логіну та пароля.</li>
...
<div id="lab9">
  ...
  <ol>
<li>Для чого використовується версійність API?<br>
Версійність API використовується для забезпечення сумісності між серверною та клієнтською частинами. Вона дозволяє розробникам оновлювати або додавати нові функції в API без того, щоб порушувати роботу програм, які вже використовують API. </li>
<li>Що таке контракт між серверною і клієнтською частиною?<br>
Контракт між серверною та клієнтською частинами - це набір правил, які визначають спосіб взаємодії між ними. Контракт визначає формат даних, які передаються між сервером та клієнтом, та типи запитів, які можна виконувати. </li>
<li>Які є шляхи додавання версійності до проекту ASP.NET WebAPI?<br>
Шляхи додавання версійності до проекту ASP.NET WebAPI включають в себе використання аннотацій, namespace'ів та в Program.cs</li>
<li>У яких випадках необхідно додавати версійність?<br>
Версійність API необхідна, коли розробник планує додавати нові функції або модифікувати наявні і коли ці зміни істотно впливають на використання API клієнтами. Тому, якщо ми просто додаємо нове поле - створювати нову версію не можна, але якщо змінюємо тип повертаємого значення - необіхдно.</li>
</ol>
</div>
  ...
  <ol>
<li>Принципова різниця між контейнерами та віртуальними машинами:<br>
Контейнери - ефективніше використовують ресурси, швидше стартують та легше масштабуються, використовують віртуалізацію на рівні операційної системи.
Віртуальні машини - імітують фізичні сервери, включають в себе операційну систему, бібліотеки та програми, більш вимогливі до ресурсів. Тобто ключовою відмінністю є відсутність емуляції в контейнерах, тому він є ефективнішим, адже використовує ядро ОС.</li>
<li>Складові контейнера:<br>
- Образ: включає виконуваний код, залежності, конфігураційні файли.<br>
- Контейнерний двигун: виконує створення, запуск та управління контейнерами.<br>
- Образний репозиторій: зберігає та обмінюється образами.<br>
- Конфігураційні файли: налаштовують додаток або середовище.<br>
- Мережа: дозволяє комунікувати з іншими контейнерами або зовнішніми ресурсами.</li>
<li>Docker Compose:<br>
Інструмент для визначення та управління багатоконтейнерними додатками.
Визначає конфігурацію у файлі `docker-compose.yml` з описом контейнерів, налаштувань, залежностей.
Дозволяє легко створювати та запускати багатоконтейнерні середовища.</li>
<li>Типи мережі контейнера:<br>
- Bridge (тип за замовчуванням): локальна мережа для комунікації між контейнерами та хост-системою.<br>
- Host: використовує мережевий стек хост-системи без ізоляції.<br>
- None: контейнери без доступу до мережі, тільки з хост-системою.<br>
- Custom: створюється власна мережа для з'єднання контейнерів.</li>
</ol>
</div>
